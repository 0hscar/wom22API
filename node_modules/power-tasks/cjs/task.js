...
const debug = (0, debug_1.default)('power-tasks:task');
class TaskContext {
    constructor() {
...
class Task extends strict_typed_events_1.AsyncEventEmitter {
    ...
    _startChildren() {
        ...
        const abortedTasks = this._abortedTasks = [];
        debug(this._id, this.name ? '(' + this.name + ')' : '', this.status, '_startChildren:a', children.length);
            ...
            if (!childrenLeft.size) {
                ...
                if (failedTasks.length) {
                    error = new Error(failedTasks.length +
                        ' child ' + (0, utils_js_1.plural)('task', failedTasks.length) +
                        ' has been failed');
                    error.failedTasks = failedTasks;
                }
                if (abortedTasks.length) {
                    error = new Error(abortedTasks.length +
                        ' child ' + (0, utils_js_1.plural)('task', abortedTasks.length) +
                        ' has been aborted');
